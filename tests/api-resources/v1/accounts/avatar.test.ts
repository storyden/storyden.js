...
describe('resource avatar', () => {
  test('create: only required params', async () => {
    const responsePromise = client.v1.accounts.avatar.create(
  ...
  test('create: required and optional params', async () => {
    const response = await client.v1.accounts.avatar.create(
      await toFile(Buffer.from('# my file contents'), 'README.md'),
    );
  });
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      client.v1.accounts.avatar.retrieve('southclaws', { path: '/_stainless_unknown_path' }),
